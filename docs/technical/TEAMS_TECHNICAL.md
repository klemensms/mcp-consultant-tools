...
Output volume is the primary design constraint: a busy channel will exhaust a context window. All reads default to 20 messages and clamp to Graph's maximum of 50 (`clampTop()`). Bodies are flattened to plain text by `htmlToText()` and share a single 30,000-character budget per read, allocated by `allocateBodyBudgets()` in `tools/format-messages.ts` and applied by `truncateText()`. The budget is shared rather than per-message, so a one-message read can spend all of it on that message while a 50-message read still cannot exceed the total. Shorter messages are served first and release what they do not use, so one long message in an otherwise ordinary read comes back whole. A body that still does not fit says how to get the rest.
...
**`textContent` discards an anchor's `href`.** Only the label survives, and it reads as ordinary prose, so a message that lost a URL looks intact and nothing signals the loss - the reader has to open Teams to recover it. `htmlToText` renders anchors as `[label](href)`. Where the label is already the URL, which is what Teams emits for a pasted link, it renders as the bare URL rather than `[https://x](https://x)`; the comparison sets aside a `mailto:`/`tel:` scheme and a trailing slash. An anchor with no `href` renders as its label, one with no label as its URL, and one whose label merely resembles a different URL still renders as a markdown link, so the real target is never hidden behind a plausible label.

**An attachment's identity lives outside the body.** The body carries only `<attachment id="...">`; the file name, the `contentUrl` a preview card points at, and the message a reply is quoting are all in the sibling `attachments[]` array. `htmlToText(html, contentType, mentions, attachments)` joins them by `id`, so a file or unfurled link renders as `[attachment: Report.docx - https://…]` and a `messageReference` as `[quoted reply from Jane Doe: the quoted text]`, clipped at 200 characters. Without the array every attachment of every kind renders as an identical `[attachment]`, which makes a link-preview card indistinguishable from a quoted reply. **A quoted reply's text exists nowhere else in the response** - it is carried in the attachment's `content` as a JSON string (`messagePreview`, `messageSender.user.displayName`), so not reading it leaves the reader inferring from chronology which message a reply answered. Same plumbing requirement as `mentions[]`: `toMessageInfo()` and `toSearchHit()` both pass their attachments through. An attachment Graph sends with no matching placeholder is appended on its own line rather than dropped.
