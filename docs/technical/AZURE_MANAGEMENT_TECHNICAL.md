...
**`list-event-grid-topics`** - List Event Grid topics. Counts both custom and system topics; lists system topics only on request.
...
| Field | Meaning |
|-------|---------|
| `total` | Topics that exist in scope, listed or not |
| `listed` | Entries in `topics` |
| `custom` / `system` | True counts per type, regardless of `includeSystemTopics` |
| `systemTopicsListed` | Whether system topics appear in `topics` |
| `note` | Present only when topics were counted but not listed |
...
- **Event Grid system topics are counted but not listed by default:** `list-event-grid-topics` keeps GUID-named system topics out of `topics` to reduce response size, while still enumerating them so `summary.total` and `summary.system` are true. That is one extra ARM list call per invocation, deliberately paid so a partial scope cannot report a clean zero.
