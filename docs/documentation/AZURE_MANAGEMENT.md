...
- **`list-virtual-machines` does not collect power state unless you ask.** Power state is not in the ARM list response, so without `includeStatus` no row carries `powerState`, every VM sits in the `not collected` bucket, and a deallocated VM is indistinguishable from a running one. `summary.note` says so. With `includeStatus` it costs one extra ARM call per VM, a refused call lands in the `unavailable` bucket rather than any power state, and every VM lands in exactly one bucket.
- **`list-scheduled-query-rules` is a different surface from `list-alert-rules`.** Log-search alerts, metric alerts and smart detectors are three disjoint provider surfaces and no one of them is the alerting configuration. Quote `summary.alerting`, not `summary.total`: a disabled rule fires nothing, a `LogToMetric` rule emits a metric instead of alerting, and a rule with no action group notifies nobody. Each is counted separately.
- **`list-logic-app-workflows` withholds `definition` and `parameters` by default** and names them in each row's `propertiesWithheld`, so their absence is never evidence a workflow has none. `triggerNames`, `actionCount` and `parameterNames` are derived before withholding. Quote `summary.enabled` rather than `summary.total` as live integration.
- **`list-api-connections` sweeps every resource group, because ARM has no subscription-wide list for `Microsoft.Web/connections`.** The count is only as complete as that sweep: `summary.complete` is false and `fanOut.failures` names the group whenever one refuses. A connection whose status is not `Connected` is counted in `summary.broken` with the reason in `statusError`, because a workflow using it fails at run time while still reporting state `Enabled`. `parameterValues` is redacted to its keys unless `AZURE_REDACT_SECRETS=false`.
