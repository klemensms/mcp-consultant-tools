...
- **Three-tier permission model:** 36 tools are always available (read-only). Upsert tools (29) require the relevant `_UPSERT` flag. Delete/disable tools (10) require the relevant `_DELETE` or `_DISABLE` flag. Each resource category has its own independent flag pair.
