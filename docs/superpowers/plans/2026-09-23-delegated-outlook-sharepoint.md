...
- 2026-09-23 21:55, monitor: build chain closed at `c37b6ba`; every builder and fix tab verified and closed, scratch files swept. Two things remain, both held in the maintainer's private task for the registration request rather than here: (1) raise the request after the 24 September discussion; (2) when IT returns the two client ids, switch the local `sharepoint` and `outlook` entries to the new registrations, sign in to Outlook once, and run the Task 6 live check (read tools first, then each switch group on its own).
