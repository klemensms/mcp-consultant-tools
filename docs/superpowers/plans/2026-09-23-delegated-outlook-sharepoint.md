...
- [ ] **Live check with switches on** (`SHAREPOINT_ENABLE_WRITE=true`, `SHAREPOINT_ENABLE_DELETE=true`), all inside one folder named `mcp-delegated-test-<yyyymmdd-hhmm>` at the root of the test site's default library: create folder, upload a small text file, rename it, copy it, move the copy into a subfolder, download it, delete the copy with `confirm: true`, then delete the test folder. Confirm the folder is gone at the end.
