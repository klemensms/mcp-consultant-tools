 ...
 *
 * The package is provider-agnostic: CODE_REVIEW_PROVIDER selects Azure DevOps (PAT), GitHub
 * Enterprise (PAT), or a GitHub App. NuGet lookups go to the public nuget.org API and need no
 * credential.
 */
...
export function buildCodeReviewConfig(env: NodeJS.ProcessEnv): CodeReviewConfig {
  ...
  if (provider === 'azure-devops') {
    ...
    if (!env.CODE_REVIEW_AZDO_PAT) missing.push('CODE_REVIEW_AZDO_PAT');
...
export function createServiceContext(): ServiceContext {
  ...
  function getClient(): CodeReviewClient {
    if (!client) {
      ...
      client = new CodeReviewClient(config, appAuth);
      // Never log tokens, org names, or base URLs — they land in transcripts.
      console.error(`Code-review client initialized (provider: ${config.provider})`);
    }
    return client;
  }
