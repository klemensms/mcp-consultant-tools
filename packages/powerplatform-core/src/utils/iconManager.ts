...
export class IconManager {
  ...
  suggestIcons(entityName: string): IconSuggestion[] {
    ...
    const lowerName = entityName
      .toLowerCase()
      .replace(new RegExp(`^${prefix}`), '') // Remove publisher prefix
  ...
  private constructFluentIconPath(fileName: string): string {
    ...
    if (!size || !style || isNaN(parseInt(size))) {
      throw new Error(
        `Invalid icon filename format: ${fileName}. Expected format: {icon_name}_{size}_{style}.svg (e.g., people_community_24_filled.svg)`
      );
    }
  ...
  async fetchIcon(fileName: string): Promise<string> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to fetch icon from GitHub: ${response.status} ${response.statusText}. URL: ${url}`
        );
      }
  ...
  generateWebResourceName(entitySchemaName: string, iconName: string): string {
    const prefix = getPublisherPrefix();
    const cleanEntityName = entitySchemaName.toLowerCase().replace(new RegExp(`^${prefix}`), '');
  ...
  buildIconUrl(iconName: string, size: number = 24, style: 'filled' | 'regular' = 'filled'): string {
    ...
    try {
      ...
      throw new Error(
        `Failed to build icon URL for '${iconName}': ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
