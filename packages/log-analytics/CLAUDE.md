...
**Two surfaces writing the same KQL is how the invalid query got in, and it happened twice.** `error-summary` shipped a query naming a column no table carries because the four shapes existed twice, inline in `cli/commands/query-commands.ts` and again in `tools/function-tools.ts`. `investigate-sync` was the same shape: four queries plus the workspace-name parse, duplicated across both surfaces, and the copies had already drifted - one rendered its empty-result note with a tick and the other without, and an unparseable workspace id `process.exit(1)`-ed in the CLI while returning an `isError` payload in the tool. Both now go through `LogAnalyticsService.investigateSync`, with the queries in `src/utils/sync-investigation-query.ts` and the report in `formatInvestigateSyncMarkdown`. **A new investigation command belongs in the service with its queries in a `utils/` builder, never inline in a surface** - a string that exists twice is a string no test can assert against.

**`executeQuery` retries the transient set, but deliberately not 504.** `src/utils/retry.ts` retries `[429, 500, 502, 503]` with exponential backoff and honours `Retry-After`. `azure-management`'s `ArmClient` and `azure-defender`'s `DefenderClient` both include 504; this package does not, because on the query API a 504 means the query itself did not finish inside the service's window, so an identical retry costs another 30-second wait and returns the same answer. The existing 504 message already says the useful thing - narrow the range or simplify the query - and retrying just delays the caller from reading it. The policy lives in this package rather than in `core` because `log-analytics` pins `core` at `33.0.0` and carries a vendored copy of that version, so a helper hoisted into `core` would be invisible here until the pin moves and the stale copy is deleted.
