...
/**
 * Column presets for different use cases.
 * - minimal: Quick overview, reduces token count significantly
 * - investigation: Error investigation with exception details
 * - full: All columns (null = no filtering)
 */
export const COLUMN_PRESETS: Record<string, string[] | null> = {
...
/**
 * Table type definition for Log Analytics results
 */
...
/**
 * Filter table columns based on a list of column names.
 * Returns a new table with only the specified columns.
 * If columnsToInclude is null, returns the original table.
 */
export function filterColumns(
  table: LogAnalyticsTable,
  columnsToInclude: string[] | null
): LogAnalyticsTable {
  if (!columnsToInclude || !table || !table.columns) {
    return table;
  }

  // Find indices of columns to include (preserve order of columnsToInclude)
  const columnIndices: number[] = [];
  const filteredColumns: { name: string; type: string }[] = [];

  for (const colName of columnsToInclude) {
    const index = table.columns.findIndex(c => c.name === colName);
    if (index !== -1) {
      columnIndices.push(index);
      filteredColumns.push(table.columns[index]);
    }
  }

  // If no matching columns found, return original table
  if (columnIndices.length === 0) {
    return table;
  }

  // Filter rows to include only the selected columns
...
/**
 * Resolve column preset or custom columns to an array of column names.
 * Returns null if no filtering should be applied.
 */
export function resolveColumnPreset(
  preset?: string,
  customColumns?: string[]
): string[] | null {
  // Custom columns take precedence
