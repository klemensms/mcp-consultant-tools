...
export class LogAnalyticsService {
  ...
  async searchLogs(
    ...
    if (tableName) {
      query = `
        search in (${tableName}) "${searchText}"
        | where TimeGenerated > ago(${this.convertTimespanToKQL(timespan)})
        | order by TimeGenerated desc
        | take ${limit}
      `.trim();
  ...
  async getFunctionLogs(
    ...
    if (severityLevel !== undefined) {
      query += `\n      | where LevelId >= ${severityLevel}`;
    }

    query += `
      | order by TimeGenerated desc
      | take ${limit}
      | project TimeGenerated, FunctionName, Message, Level, ExceptionDetails, HostInstanceId
    `.trim();
  ...
  async getFunctionErrors(
    ...
    query += `
      | order by TimeGenerated desc
      | take ${limit}
      | project TimeGenerated, FunctionName, Message, ExceptionDetails, Level, HostInstanceId
    `.trim();
