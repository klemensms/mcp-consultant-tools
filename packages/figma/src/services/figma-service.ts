...
import type { FigmaConfig, FigmaDataOptions, FigmaNodeImageOptions, FigmaImagesApiResponse } from "../models/index.js";
...
export class FigmaService {
  ...
  /**
   * Get rendered image URLs for specific nodes via Figma Images API.
   * Returns a map of nodeId -> temporary CDN URL (valid ~14 days).
   */
  async getNodeImages(
    fileKey: string,
    nodeIds: string[],
    options?: FigmaNodeImageOptions,
  ): Promise<Record<string, string>> {
    const format = options?.format ?? 'png';
    const scale = options?.scale ?? 2;
    const ids = nodeIds.join(',');
    const endpoint = `/images/${fileKey}?ids=${ids}&format=${format}&scale=${scale}`;

    const response = await this.request<FigmaImagesApiResponse>(endpoint);

    if (response.err) {
      throw new Error(`Figma Images API error: ${response.err}`);
    }

    // Filter out null values (nodes that couldn't be rendered)
    const result: Record<string, string> = {};
    for (const [nodeId, url] of Object.entries(response.images)) {
      if (url) result[nodeId] = url;
    }
    return result;
  }

  /**
   * Download a rendered image from a CDN URL to local disk.
   * Uses the existing downloadFigmaImage utility (no auth needed for CDN URLs).
   */
  async downloadNodeImage(
    ...
    const { downloadFigmaImage } = await import('../figma/utils/common.js');
